...
const getAllBurgers = async (req, res) => {
    const burgers = await Burger.find({createdBy: req.user.userID}).sort("created at")
    res.status(StatusCode.OK).json({burgers, length: burgers.length})
}
...
const getBurger = async (req, res) => {
    ...
    if(!burger){
        throw new NotFoundError(`not job wiht id  ${burgerID}`)
    }
...
const updateBurger = async (req, res) => {
    ...
    const burger = await Burger.findByIdAndUpdate(
        ...
        if(!burger){
        throw new BadRequestError(`no job with id ${burgerID}`)
        }
...
const deleteBurger = async (req, res) => {
   ...
   if(!burger){
       throw new BadRequestError(`no job with id ${burgerID} found`)
   }
}
